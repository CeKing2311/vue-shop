    ...
    <el-card>
        ...
        <el-tabs tab-position="left" v-model="activeIndex" :before-leave="beforeTabLeave" @tab-click="tabClicked">
            ...
            <el-tab-pane label="商品内容" name="4">
              ...
              <el-button type="primary" class="btn-top-15" @click="saveGoods">保存商品</el-button>
            </el-tab-pane>
        </el-tabs>
        </el-form>
    </el-card>
  ...
  methods: {
    async getGoodsInfo() {
      ...
      res.data.pics.forEach(item => {
        const obj = {
          pic: item.pics_mid_url.replace("mid_", "")
        };
        picList.push(obj);
      });
      res.data.pics = picList;
    ...
    async tabClicked() {
      ...
      if (this.activeIndex === "1" && this.manyTableData.length === 0) {
        ...
        res.data.forEach(item => {
          ...
          if (attr !== undefined) {
            item.attr_value = attr.attr_value ? attr.attr_value.split(" ") : [];
          } else {
            item.attr_value = [];
          }
        });
        ...
        this.goodsForm.attrs.forEach(item => {
          res.data.forEach(d => {
            if (d.attr_id == item.attr_id) {
              d.attr_vals = item.attr_value;
            }
          });
        });
